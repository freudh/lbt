...
class Trainer:
    def __init__(self, model, dataset, logger, logdir,
        ...
        self.logger.info('lr %f decay %f in %s epoch' % (
        ...
        # self.lr = tf.Variable(lr, tf.float32)
        self.lr = lr
        self.momentum = tf.Variable(momentum, tf.float32)
        # self.lr_decay_epoch = [int(s) for s in lr_decay_epoch.split('-')]
        ...
        self.train_op = self.optimizer.apply_gradients(self.model.grads_and_vars(), self.global_step)
    ...
    def train(self):
        ...
        for epoch in range(self.n_epoch):
            ...
            #     self.logger.info('Decay lr by %f' % self.lr_decay_factor)
            #     self.sess.run(tf.assign(self.lr, self.lr*self.lr_decay_factor))
            ...
            while True:
                try:
                    ...
                    b += 1
                    _, _, loss, acc, summary, step = self.sess.run([self.train_op, self.update_op,
            ...
            while True:
                try:
                    ...
                    batch_cnt += 1
                    acc, loss = self.sess.run([self.model.accuracy, self.model.loss],
                        feed_dict={self.model.input_X: X, self.model.input_y: y})
                    test_acc += acc
                    test_loss += loss
                except tf.errors.OutOfRangeError:
            ...
            test_acc /= batch_cnt
            test_loss /= batch_cnt
            self.logger.info('Epoch %d test accuracy %f' % (1+epoch, test_acc))
